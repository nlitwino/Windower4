...
function job_post_precast(spell,action,spellMap,eventArgs)
  ...
  if spell.skill:contains('Magic') then
    if state.IdleMode.value ~= 'Combat' and state.CastingMode.value ~= 'Combat' then
      sets.postPrecast = set_combine(sets.postPrecast,{main="Vivid strap +1"})
    end
  end
...
function job_post_midcast(spell,action,spellMap,eventsArgs)
  ...
  if spell.skill == 'Enhancing Magic' or (spell.skill == 'Healing Magic' and not (spellMap == 'Cure' or spellMap == 'Curaga')) then
    if player.status == 'Idle' and state.IdleMode.value == 'Normal' and state.CastingMode.value ~= 'Combat' then
      sets.postMidcast = set_combine(sets.postMidcast,{main="Seveneyes",sub="Muse tariqah"})
    end
  elseif (spellMap == 'Cure' or spellMap == 'Curaga') and (state.IdleMode.value ~= 'Combat' and state.CastingMode.value ~= 'Combat') then
    sets.postMidcast = set_combine(sets.postMidcast,{main="Chatoyant staff",sub="Raptor leather strap +1"})
  else
    if player.status == 'Idle' and state.IdleMode.value == 'Normal' and state.CastingMode.value ~= 'Combat' then
      ...
      if spell.skill == 'Enfeebling Magic' then
        ...
        if state.CastingMode.value == 'Normal' then
          sets.postMidcast = set_combine(sets.postMidcast,{main="Alkalurops"})
        else
          sets.postMidcast = set_combine(sets.postMidcast,{main="Chatoyant staff"})
        end
        
        if spell.type == 'WhiteMagic' then
          sets.postMidcast = set_combine(sets.postMidcast,{sub="Raptor leather strap +1"})
        elseif spell.type == 'BlackMagic' then
          sets.postMidcast = set_combine(sets.postMidcast,{{sub="Bugard leather strap +1"}})
        end 
        
      end
      
      if state.CastingMode.value == 'Acc' and player.status ~= 'Engaged' then
          sets.postMidcast = set_combine(sets.postMidcast,sets.grip[spell.element])
      end
  ...
  if spell.skill ~= 'Enhancing Magic' or spell.skill ~= 'Enfeebling Magic' then
    if spell.element == world.weather_element or spell.element == world.day_element then
      ...
      if world.weather_element ~= 'Dark' and spellMap == 'Cure' then
        sets.postMidcast = set_combine(sets.postMidcast,{waist="Hachirin-no-obi"})
      end
